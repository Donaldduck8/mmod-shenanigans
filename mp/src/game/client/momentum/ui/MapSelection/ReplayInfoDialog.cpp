...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDialogReplayInfo::CDialogReplayInfo(vgui::Panel *parent, const char *mapname) : Frame(parent, "DialogMapInfo")
{
    ...
    LoadControlSettings("resource/ui/DialogReplayInfo.res");
    ...
    {
        Assert("Nullptr pointers on CDialogReplayInfo");
    }
    // set the defaults for sorting
    // hack, need to make this more explicit functions in ListPanel
    if (m_pPlayerList)
    {
        m_pPlayerList->AddColumnHeader(0, "PlayerName", "#MOM_Name", 100);
        m_pPlayerList->AddColumnHeader(1, "Date", "Date", 140);
        m_pPlayerList->AddColumnHeader(2, "Time", "Time", 84);
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CDialogReplayInfo::~CDialogReplayInfo()
{
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::Run(const char *titleName)
{
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::PerformLayout()
{
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::Connect()
{
    OnConnect();
}
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::OnConnect()
{
...
ConVar current_replay("current_replay", "", FCVAR_HIDDEN | FCVAR_ARCHIVE);
void CDialogReplayInfo::ApplyConnectCommand(const char *mapName)
{
    KeyValues *kv = m_pPlayerList->GetItem(m_pPlayerList->GetSelectedItem(0));
    current_replay.SetValue(kv->GetString("replayPath"));
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::ConnectToServer()
{
    m_bConnecting = false;
    // close this dialog
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::RefreshComplete(EMatchMakingServerResponse response)
{
}
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::ClearPlayerList()
{
...
//-----------------------------------------------------------------------------
void CDialogReplayInfo::AddPlayerToList(KeyValues* pPlayerInfo)
{
...
void CDialogReplayInfo::GetMapInfo(const char* mapname)
{
    if (steamapicontext && steamapicontext->SteamHTTP())
    {
        ...
        if (steamapicontext->SteamHTTP()->SendHTTPRequest(handle, &apiHandle))
        {
            ...
            cbGetMapInfoCallback.Set(apiHandle, this, &CDialogReplayInfo::GetMapInfoCallback);
        }
        else
        {
...
void CDialogReplayInfo::GetMapInfoCallback(HTTPRequestCompleted_t *pCallback, bool bIOFailure)
{
...
void CDialogReplayInfo::Get10MapTimes(const char* mapname)
{
    if (steamapicontext && steamapicontext->SteamHTTP())
    {
        char szURL[BUFSIZ];
        ...
        if (steamapicontext->SteamHTTP()->SendHTTPRequest(handle, &apiHandle))
        {
            cbGet10MapTimesCallback.Set(apiHandle, this, &CDialogReplayInfo::Get10MapTimesCallback);
        }
        else
        {
...
void CDialogReplayInfo::GetReplayInfo(const char *mapname) 
{
    ...
    KeyValues *kv = new KeyValues("replayData");
    const char *pFoundFile = filesystem->FindFirstEx(path, "MOD", &found);
    while (pFoundFile)
    {
        g_pMomentumUtil->CreateScoreKeyvalues(pFoundFile, kv);
    ...
    //Msg("What is this %s", kv->GetFirstSubKey());

    for (KeyValues *pKey = kv->GetFirstTrueSubKey(); pKey; pKey = pKey->GetNextTrueSubKey())
    {
        m_pPlayerList->AddItem(pKey, 0, false, true);
        //Msg(pKey->GetString("replayPath"));
    }
}

void CDialogReplayInfo::Get10MapTimesCallback(HTTPRequestCompleted_t *pCallback, bool bIOFailure)
{
    Msg("We're hitting this callback.");
    if (bIOFailure)
    {
        Warning("%s - bIOFailure is true!\n", __FUNCTION__);
        return;
