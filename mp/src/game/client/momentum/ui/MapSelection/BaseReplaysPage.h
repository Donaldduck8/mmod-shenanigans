...
#ifndef BASEREPLAYSPAGE_H
#define BASEREPLAYSPAGE_H
#ifdef _WIN32
#pragma once
#endif
...
//-----------------------------------------------------------------------------
class CBaseReplaysPage : public vgui::PropertyPage, public IReplayList
{
    DECLARE_CLASS_SIMPLE(CBaseReplaysPage, vgui::PropertyPage);

public:
    CBaseReplaysPage(vgui::Panel *parent, const char *name, const char *pCustomResFilename = nullptr);
    ~CBaseReplaysPage();
    ...
    virtual replaystruct_t *GetMap(unsigned int serverID);
    ...
    //STEAM_CALLBACK(CBaseReplaysPage, OnFavoritesMsg, FavoritesListChanged_t, m_CallbackFavoritesMsg);
...
protected:
    ...
    void RemoveMap(replaydisplay_t&);

    //MOM_TODO: Correlate this to online maps
    virtual bool BShowMap(replaydisplay_t &server) { return server.m_bDoNotRefresh; }
    ...
    virtual bool CheckPrimaryFilters(replaystruct_t &);
    virtual bool CheckSecondaryFilters(replaystruct_t &);
    virtual bool CheckTagFilter(replaystruct_t &) { return true; }
